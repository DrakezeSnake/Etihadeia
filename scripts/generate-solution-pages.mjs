...
<html lang="en">
  ...
  <body data-page="product-document" data-slug="${escapeHtmlAttr(slug)}">
    <div id="app"></div>
    <script type="module" src="/src/solutions/main.js"></script>
  </body>
</html>
...
for (const doc of productDocumentsMod.productDocuments) {
  const dir = path.join(root, "solutions", "documents", doc.slug);
  fs.mkdirSync(dir, { recursive: true });
  fs.writeFileSync(path.join(dir, "index.html"), documentStubHtml(doc.slug, doc.title, doc.description), "utf8");
}

console.log("Wrote product document pages:", productDocumentsMod.productDocuments.map((x) => x.slug).join(", "));
...
console.log(productDocumentsMod.productDocuments.map((x) => `        document_${x.slug.replace(/-/g, "_")}: resolve(__dirname, "solutions/documents/${x.slug}/index.html"),`).join("\n"));
